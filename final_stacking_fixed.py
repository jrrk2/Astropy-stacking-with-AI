...
including 180-degree rotation for dark frames, and APP-like registration.
"""
...
def process_fits_files(fits_directory, transformations, output_file, 
    ...
    # Create output directories for intermediates if needed
    if save_calibrated:
        ...
        print(f"Created directories for intermediate files:")
        print(f"  - {calibrated_dir}")
        print(f"  - {transform_dir}")
    ...
    for transform_info in tqdm(valid_transforms, desc="Processing frames"):
        ...
        # Save original frame before calibration if requested
        if save_calibrated:
    ...
    ref_dark_data, message = get_dark_frame(reference_file, dark_frame_path, dark_manager, rotate_dark_180)
    ...
    new_hdu.header['DARKROT180'] = str(rotate_dark_180)
    ...
    with open(info_file, 'w') as f:
        ...
        f.write(f"Dark frame 180-degree rotation: {rotate_dark_180}\n")
...
def main():
    ...
    parser.add_argument('--no-dark-rotation', action='store_true',
                      help='Disable 180-degree rotation for dark frames')
    ...
    process_fits_files(
        ...
        rotate_dark_180=not args.no_dark_rotation, 
