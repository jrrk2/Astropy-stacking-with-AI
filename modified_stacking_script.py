        ...
        try:
            with fits.open(fits_file) as hdul:
                ...
                if has_transform:
                    ...
                    # DEBUG: Print the extracted matrix
                    if file_idx == 0:
                    ...
                    # Ensure matrix is valid (has non-zero determinant)
                    if abs(np.linalg.det(matrix)) < 1e-10:
                        print(f"  Warning: Near-singular matrix in {os.path.basename(fits_file)}")
                        # Use identity matrix in this case
                        matrix = np.eye(3)
