...
(* Update FITS header using existing hash table functionality *)
let update_fits_header fits_path updates =
  try
    (* Get existing header *)
    ...
    let fd = open_in_bin fits_path in
    ...
    Hashtbl.iter (fun key value ->
      if key <> "END" then begin
        let record = Printf.sprintf "%-8s%s" key value in
        let len = min (String.length record) 80 in
        Bytes.blit_string record 0 new_header !pos len;
        pos := !pos + 80
      end
    ) hdrh;
    ...
    (* Write the new header back to the file *)
    let temp_path = fits_path ^ ".tmp" in
    let out_fd = open_out_bin temp_path in
    
    (* Write header *)
    output out_fd new_header 0 header_size;
    
    (* Copy data portion from original file *)
    let in_fd = open_in_bin fits_path in
    ignore (input in_fd (Bytes.create (String.length header)));  (* Skip original header *)
    ...
    Sys.rename temp_path fits_path;
    
    true
  with e ->
    Printf.eprintf "Error updating FITS header: %s\n" (Printexc.to_string e);
    false
