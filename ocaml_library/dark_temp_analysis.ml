...
let sort_dark_frames files base_dir dry_run =
  ...
  if Array.length temp_infos = 0 then
    ...
    Array.iter (fun ti -> 
      ...
      (* Generate new filename with date if available *)
      let orig_name = Filename.basename ti.filename in
      let new_name = match format_date_from_header ti.qhdr with
        | Some date_str -> 
            let extension = Filename.extension orig_name in
            sprintf "dark_%s%s" date_str extension
        | None -> orig_name in
      let new_path = sprintf "%s/%s" dir_name new_name in
...
let parse_args () =
  ...
  let specs = [
    ...
    ("-sort", Arg.Unit (fun () -> flags.sort_darks <- true),
     "Sort darks into temperature-binned directories");
