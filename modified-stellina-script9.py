...
# Import DarkFrameManager for calibration if available
try:
    from dark_manager import DarkFrameManager
    DARK_CALIBRATION_AVAILABLE = True
except ImportError:
    DARK_CALIBRATION_AVAILABLE = False
    print("Warning: dark_manager.py not found. Dark frame calibration will not be available.")
...
def get_new_filepath(fits_path, base_dir='lights', calibrated=False):
    ...
    try:
        with fits.open(fits_path) as hdul:
            ...
            fits_filename = Path(fits_path).name
            bayer_pattern = ""
            if 'r.fits' in fits_filename:
                bayer_pattern = "RGGB"
            elif 'b.fits' in fits_filename:
                bayer_pattern = "BGGR"
        ...
        prefix = "cal_" if calibrated else "light_"
        if bayer_pattern:
            filename = f"{prefix}{dt.strftime('%Y%m%d_%H%M%S')}_{bayer_pattern}.fits"
        else:
            filename = f"{prefix}{dt.strftime('%Y%m%d_%H%M%S')}.fits"
...
def calibrate_fits(fits_path, dark_manager):
    """Apply dark frame calibration to FITS file"""
    try:
        # Open the FITS file
        with fits.open(fits_path) as hdul:
            # Get temperature and Bayer pattern
            header = hdul[0].header
            temp = header.get('TEMP')
            
            # Get Bayer pattern from header or filename
            bayer_pattern = header.get('BAYERPAT')
            if not bayer_pattern:
                fits_filename = Path(fits_path).name
                if 'r.fits' in fits_filename or '_RGGB' in fits_filename:
                    bayer_pattern = 'RGGB'
                elif 'b.fits' in fits_filename or '_BGGR' in fits_filename:
                    bayer_pattern = 'BGGR'
                else:
                    bayer_pattern = 'RGGB'  # Default
                
            # Load light data
            light_data = hdul[0].data.astype(np.float32)
            
            # Get appropriate dark frame
            dark_data, info_msg = dark_manager.get_dark_frame(temp, bayer_pattern, light_data)
            print(f"  Calibration info: {info_msg}")
            
            # Perform dark subtraction with minimum value of 0
            calibrated_data = np.maximum(light_data - dark_data, 0)
            
            # Create new HDU with calibrated data
            new_hdu = fits.PrimaryHDU(calibrated_data.astype(np.uint16))
            
            # Copy header from original - safely handling potentially problematic values
            for key in header:
                if key not in ['SIMPLE', 'BITPIX', 'NAXIS', 'NAXIS1', 'NAXIS2']:
                    try:
                        new_hdu.header[key] = header[key]
                    except ValueError as e:
                        # Handle problematic values by trying to sanitize them
                        # or skipping if we can't fix them
                        value = header[key]
                        if isinstance(value, str):
                            # For string values, try to properly format them
                            try:
                                # If it's a string that might need quotes, ensure it's properly quoted
                                new_hdu.header[key] = f"{value}"
                            except ValueError:
                                # If still failing, skip this header
                                print(f"  Warning: Skipping problematic header {key}={value}")
                                continue
                        else:
                            # For non-string values, just skip
                            print(f"  Warning: Skipping problematic header {key}={value}")
                            continue
            
            # Add calibration info
            new_hdu.header['DARKSUB'] = (True, 'Dark frame subtracted')
            new_hdu.header['DARKTEMP'] = (temp, 'Temperature of light frame (C)')
            new_hdu.header['BAYERPAT'] = (bayer_pattern, 'Bayer pattern')
            
            return new_hdu, True
            
    except Exception as e:
        print(f"  Error calibrating {fits_path}: {e}")
        import traceback
        traceback.print_exc()
        return None, False
...
def process_directory(src_dir, base_dir='lights', target_name=None, max_separation_deg=1.0, 
                     ...
                     calibrate=False, darks_dir=None):
    ...
    if target_name:
        ...
        if target_coords is None:
            ...
            return 0, 0, 0, 0, 0, 0  # Return all six counters
    ...
    # Initialize dark frame manager if calibration is requested
    dark_manager = None
    if calibrate and darks_dir:
        if not DARK_CALIBRATION_AVAILABLE:
            print("ERROR: Dark calibration requested but dark_manager.py is not available")
            return 0, 0, 0, 0, 0, 0
            
        try:
            print(f"\n=== Initializing Dark Frame Manager ===")
            print(f"Loading dark frames from: {darks_dir}")
            dark_manager = DarkFrameManager(darks_dir)
            print(f"Dark manager initialized with temperature range: {dark_manager.temp_range[0]}-{dark_manager.temp_range[1]}K")
            print("=== Dark Manager Initialization Complete ===\n")
        except Exception as e:
            print(f"ERROR: Failed to initialize dark frame manager: {e}")
            print("Continuing without calibration")
            dark_manager = None
            calibrate = False
    elif calibrate:
        print("ERROR: Dark calibration requested but no darks directory specified")
        print("Use --darks-dir to specify the directory containing dark frames")
        calibrate = False
    ...
    # Initialize counters for specific operations
    processed = 0
    skipped = 0
    errors = 0
    calibrated = 0
                    ...
                    if not new_path.exists():
                        ...
                        if transformations and add_registration:
                            ...
                            if reg_success:
                                ...
                                registration_added += 1  # Increment counter for successful registration
                            else:
                                print(f"  Failed to add registration attributes")
                                registration_failed += 1  # Increment counter for failed registration
                        
                        # Apply calibration if requested
                        if calibrate and dark_manager:
                            print(f"  Applying dark frame calibration...")
                            new_hdu, cal_success = calibrate_fits(new_path, dark_manager)
                            if cal_success:
                                print(f"  Successfully calibrated frame")
                                calibrated += 1  # Increment counter for successful calibration
                                # Save calibrated HDU
                                cal_path, _ = get_new_filepath(new_path, base_dir, calibrated=True)
                                cal_path.parent.mkdir(parents=True, exist_ok=True)
                                new_hdu.writeto(cal_path, overwrite=False)
                                print(f"  Saved calibrated frame to {cal_path}")
                            else:
    ...
    # Add detailed counts for specific operations
    if calibrate:
        print(f"  Calibrated frames: {calibrated}")
    
    if add_registration:
        print(f"  Registration attributes added: {registration_added}")
        print(f"  Registration attributes failed: {registration_failed}")
    
    return processed, skipped, errors, calibrated, registration_added, registration_failed
...
if __name__ == "__main__":
    ...
    parser.add_argument('--calibrate', action='store_true',
                      help='Apply dark frame calibration to light frames')
    parser.add_argument('--darks-dir',
                      help='Directory containing dark frames for calibration')
    ...
    # Check for dark calibration request
    if args.calibrate and not args.darks_dir:
        print("Error: Calibration requested but no darks directory specified")
        print("Please use --darks-dir to specify the directory containing dark frames")
        sys.exit(1)
    
    # Capture all six return values
    processed, skipped, errors, calibrated, registration_added, registration_failed = process_directory(
        args.directory, 
        args.output, 
        args.target, 
        args.max_separation,
    ...
    if args.calibrate:
        ...
        if total_frames > 0:
            cal_percentage = calibrated / total_frames * 100
            print(f"- Successfully calibrated frames: {calibrated}/{total_frames} ({cal_percentage:.1f}% of total)")
    
    if not args.no_registration:
        total_frames = processed + skipped
        if total_frames > 0:
            reg_percentage = registration_added / total_frames * 100
            print(f"- Successfully added registration data: {registration_added}/{total_frames} ({reg_percentage:.1f}% of total)")
            if registration_failed > 0:
                print(f"- Failed to add registration data: {registration_failed}/{total_frames} ({registration_failed/total_frames*100:.1f}% of total)")
