...
def parse_observation_json(log_file_path):
    ...
    try:
        ...
        print("Error parsing JSON. Check the observation.json file format.")
        return []
...
def find_observation_json(fits_path):
    """
    Search for observation.json file in the parent directory of the FITS file
    ...
    """
    # Get parent directory of the FITS file
    parent_dir = Path(fits_path).parent
    
    # Look for observation.json in parent directory
    observation_json_path = parent_dir.parent / "observation.json"
    
    if observation_json_path.exists():
        print(f"Found observation.json: {observation_json_path}")
        return observation_json_path
    else:
        print(f"No observation.json found in parent directory: {parent_dir.parent}")
        return None

def annotate_fits_with_registration(fits_path, transformations):
    """
    ...
    # Find matching transformation by comparing filenames
    ...
    for transform in transformations:
        ...
        if transform_filename == fits_filename:
            matching_transform = transform
            break
    
    if not matching_transform:
        print(f"No matching transformation found for {fits_filename}")
        # Try matching by index instead of filename
        for transform in transformations:
            # Check if file index matches the one in the FITS filename
            file_index = transform.get('file_index', -1)
            if file_index != -1:
                match = re.search(r'img-(\d{4})', fits_filename)
                if match and match.group(1) == str(file_index).zfill(4):
                    matching_transform = transform
...
def process_directory(src_dir, base_dir='lights', target_name=None, max_separation_deg=1.0, 
                     dry_run=True, solve=False, add_registration=True):
    ...
    observation_json_path = find_observation_json(src_dir)
                ...
                try:
                    ...
                    if not new_path.exists():
                        ...
                        if transformations and add_registration:
                            reg_success = annotate_fits_with_registration(new_path, transformations)
                            if reg_success:
