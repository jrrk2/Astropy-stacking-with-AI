...
class MmapStacker:
    ...
    def _update_channel_sums(self, color, data):
        ...
        if IS_APPLE_SILICON:
            current = self.channel_sums[color]
            if current is None:
                self.channel_sums[color] = data.copy()
            else:
                self.channel_sums[color] = current + data
        else:
    ...
    def process_files(self, files):
        ...
        if IS_APPLE_SILICON:
            # Initialize channel sums with zeros after first frame gives shape
            with fits.open(files[0]) as hdul:
                shape = hdul[0].data.shape
                for color in ['R', 'G', 'B']:
                    self.channel_sums[color] = np.zeros(shape, dtype=np.float32)
        ...
        if IS_APPLE_SILICON:
            ...
            for color in ['R', 'G', 'B']:
                ...
                if channel_data is None or count == 0:
