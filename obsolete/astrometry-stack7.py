...
class AstrometricStacker:
    ...
    def process_frames(self, files):
        ...
        # Pre-allocate memory for channels - use correct shape from debayered data
        first_r, _, _ = results[0]['data']
        ref_shape = first_r.shape  # Use shape of debayered channel
        for color in ['R', 'G', 'B']:
            channels[color] = np.zeros((len(results), ref_shape[0], ref_shape[1]), dtype=np.float32)
            print(f"Allocated array for {color} channel: shape {channels[color].shape}")
    ...
    def debayer_data(self, data, pattern='RGGB', wcs=None):
        """Debayer data using proper demosaicing to preserve resolution"""
        from colour_demosaicing import demosaicing_CFA_Bayer_bilinear
        
        # Demosaic to full RGB
        rgb = demosaicing_CFA_Bayer_bilinear(data, pattern)
        
        # Extract channels while preserving WCS
        r = rgb[:,:,0]
        g = rgb[:,:,1]
        b = rgb[:,:,2]
