...
class StellinaProcessor:
    ...
    def clean_header(self, header):
        """Clean Stellina FITS header by removing '=' from keywords"""
        cleaned = fits.Header()
        for card in header.cards:
            key = card.keyword.rstrip('=')  # Remove trailing '=' if present
            cleaned[key] = card.value
        return cleaned
    ...
    def debayer_fits(self, filename):
        ...
        with fits.open(filename) as hdul:
            ...
            header = self.clean_header(hdul[0].header)
            pattern = header.get('BAYERPAT', 'BGGR').strip()
    ...
    def subtract_pedestal(self, ccd):
        ...
        # Get exposure time - handle Stellina's format
        exptime = float(ccd.header.get('EXPOSURE', 10000)) / 1000.0  # Convert ms to seconds
        return ccdp.subtract_dark(ccd, pedestal, exposure_time=exptime, exposure_unit=u.second)
