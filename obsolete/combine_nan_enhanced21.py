...
def solve_astrometry(image_path):
    ...
    try:
        # Run solve-field with typical options for astronomical images
        cmd = [
            'solve-field',
            '--overwrite',  # Overwrite existing files
            '--no-plots',   # Don't create debug plots
            '--no-verify',  # Skip verification step
            '--scale-units', 'arcsecperpix',
            '--scale-low', '0.5',    # Adjust these scale values based on 
            '--scale-high', '2.0',   # your telescope's pixel scale
            image_path
        ]
