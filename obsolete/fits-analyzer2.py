...
class StellanaAnalyzer:
    ...
    def analyze_frame(self, filename):
        ...
        with fits.open(filename) as hdul:
            # Handle BZERO offset properly
            data = hdul[0].data.astype(np.float32)
            bzero = hdul[0].header.get('BZERO', 0)
            data = data + bzero
            
            # Basic pedestal subtraction
            pedestal = np.percentile(data, 1)
            data = data - pedestal
            ...
            # Compare G1 and G2 patterns
            g1_fft = np.fft.fftshift(np.fft.fft2(G1))
            g2_fft = np.fft.fftshift(np.fft.fft2(G2))
            g1_power = np.log10(np.abs(g1_fft) + 1)
            g2_power = np.log10(np.abs(g2_fft) + 1)
            
            # Compute difference pattern
            pattern_diff = g2_power - g1_power
