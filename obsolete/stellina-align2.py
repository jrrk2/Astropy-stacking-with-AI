...
class EnhancedStellinaAligner:
    ...
    def detect_stars(self, data, detection_sigma=3.0):
    ...
    def align_frames(self, files, output_base='aligned'):
        ...
        for idx, filename in enumerate(tqdm(files)):
            try:
                ...
                # Enhanced transform computation
                try:
                    # First try with more strict parameters
                    transform, (source_pts, target_pts) = aa.find_transform(
                        lum,
                        ref_lum,
                        max_control_points=self.max_stars,
                        tolerance=0.5,  # Stricter position matching
                        estimation_control_points=50  # Use more points for estimation
                    )
                except Exception:
                    # Fall back to more lenient parameters
                    transform, (source_pts, target_pts) = aa.find_transform(
                        lum,
                        ref_lum,
                        max_control_points=self.max_stars,
