...
def save_16bit_png(data, filename):
    """
    Save a normalized 16-bit PNG with reduced file size
    ...
    # Convert to 8-bit with scaling
    scaled_data_8bit = (scaled_data / 256).astype(np.uint8)
    
    # Save with reduced size and quality
    cv2.imwrite(filename, scaled_data_8bit, [
        cv2.IMWRITE_PNG_COMPRESSION, 9,  # Maximum compression
        cv2.IMWRITE_PNG_STRATEGY, cv2.IMWRITE_PNG_STRATEGY_DEFAULT,
        cv2.IMWRITE_PNG_BILEVEL, 1  # Bilevel encoding
    ])
